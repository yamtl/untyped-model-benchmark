...
import benchmark.GroovyUtil
import benchmark.MemoryResult
import benchmark.PerformanceResult
...
abstract class BenchmarkHarness_csv  {
	
	var protected YAMTLModule xform
		
	val BASE_PATH = "models/physicalActivity"
	...
	def getIterations(int min, int max) {
	...
	def runBenchmark(int times, int min_iteration, int max_iteration) 
	{
		...
		resultsMemory += ", total_median (ms), init_median (ms), load_median (ms), trafo_median (ms), save_median (ms)\n"
		

		/**
		 * WARM-UP
		 */
		val fileNameWarmUp = 'dataset_1'
		val filePathWarmUp = '''«BASE_PATH»/«fileNameWarmUp».csv'''
		for (iter : 0 ..< WARM_UP_ITERATION_NUMBER) { 
		
			//////////////////////////////////////////////////////////////////////////
			// INITIALIZATION
			doInitialization(1000)
		...
		for (fileName : getIterations(min_iteration, max_iteration)) {
			val filePath = '''«BASE_PATH»/«fileName».csv'''
			...
			for (iter : 0 ..< times) { 
				...
				doInitialization( (Math.pow(10, iter+1) * 1.3) as int )
				now = System.nanoTime();
				...
				if (debug) println('''initialization time: «preProcessTime»''')
				freeGC()
